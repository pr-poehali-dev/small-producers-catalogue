...
def send_confirmation_email(to_email: str, token: str):
    """Отправляем письмо с подтверждением на email через SMTP"""
    # В данном проекте используем заглушку — логируем токен
    # При наличии SMTP credentials — отправляем настоящее письмо
    print(f"CONFIRM TOKEN for {to_email}: {token}")
    # Если нет SMTP — просто логируем, пользователь получит токен в ответе (dev mode)
...
def handler(event: dict, context) -> dict:
    """Auth: register, login, confirm-email, me, logout"""
    ...
    try:
        ...
        if method == 'POST' and action == 'register':
            ...
            role = body.get('role', 'manufacturer')  # manufacturer | visitor

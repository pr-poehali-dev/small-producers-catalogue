...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        # POST /favorites/toggle — добавить/убрать из избранного
        elif method == 'POST' and action == 'toggle':
            fav_type = body.get('type')  # 'manufacturer' | 'product'
        ...
        # GET /chat-messages?chat_id=X — сообщения чата
        ...
        # POST /send-message — отправить сообщение
            ...
            if chat_row:
                ...
                cur.execute(
                    f"SELECT user_id FROM {SCHEMA}.manufacturers WHERE id=%s",
                    (manufacturer_id,)
                )
                mfr = cur.fetchone()
                if not mfr:
